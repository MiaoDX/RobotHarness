...
这个仓库本身依然是一次 agentic engineering 实验：

- 自 0307 至今 127 commit，77 由 Claude / 50 由 MiaoDX
- 比例从 0307 的 96/4 调整到 60/40——**不是退步，是项目成熟度的信号**：发布、契约审核、最终 bless 这些必须人做
- 开发流程：Claude Opus chat 做 roadmap / refactor steering → 拆 GitHub issue → claude.ai/code/scheduled 做 issue 优先级标签 + 自动 PR → CI 跑 UT/CPU/GPU 并自动生成 8 个 demo 的 HTML 报告

> Claude 不是写几段代码的助手，而是承担项目规划和技术 steering 的工作；具体编码主要由 Claude Code 与 Codex 在云端完成。
...
距离上次 hackathon 一个多月，127 个 commit。**核心不是堆功能，是把 wedge 整体换了。**0307 时我们卖的是"给 agent 装眼睛"，那是 capability 故事。这次我们卖的是"工程师敢离开屏幕"，这是 ROI 故事。后者更难做，但落地价值高一个数量级。
